...
var prefix setget no_set_access, get_prefix

# Pool size on initialization
var size setget no_set_access, get_size

# Preloaded scene resource
var scene setget no_set_access, get_scene

# Dictionary of "alive" objects currently in-use.
# Using a dictionary for fast lookup/deletion
var alive = {} setget no_set_access, no_get_access

# Array of "dead" objects currently available for use
var dead = [] setget no_set_access, no_get_access
...
func _on_killed(target):
	print("_on_killed name = " + target.name)
